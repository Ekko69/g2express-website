<div>
    ...
    <x-baseview title="{{ __('File Upload Limits') }}">
        ...
        <x-form action="saveFileLimits" backPressed="$emitUp('goBack')">
            ...
            <div class="grid grid-cols-1 gap-1 md:gap-4 md:grid-cols-2">
                <x-input title="{{ __('Max Service/Booking Images') }}" name="max_service_images_limit"
                    type="number" />
                <x-input title="{{ __('Service/Booking Images') }} {{ __('File Size') }}"
                    name="service_image_size_limit" type="number" hint="{{ $kbHint }}" />
            </div>
            ...
            <div class="grid grid-cols-1 gap-1 md:gap-4 md:grid-cols-2">
                ...
                <x-input title="{{ __('Prescription Images') }} {{ __('File Size') }}"
                    name="prescription_file_size_limit" type="number" hint="{{ $kbHint }}" />
            </div>
            {{-- --}}
            <div class="my-4">
